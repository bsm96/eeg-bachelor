...
def compute_features(subjects, cond_code, labels, medoids, X, mode, bins):
    ...
    # Compute n_bins (B) before normalizing anything.
    # Must match the binning used in the entropy estimators above.
    if mode == "state-hist":
        B = int(bins)
    else:
        B = int(np.sqrt(medoids.shape[1]))

    # H_i per state (raw entropy; SciPy uses natural log).
    if mode == "state-hist":
        H_vec_raw = state_entropies_from_hist(X, labels, K, bins=bins)
    else:
        H_vec_raw = np.array([centroid_entropy_from_medoid(medoids[i]) for i in range(K)], float)

    # Normalize H_i by log(B) so H_i ∈ [0, 1] and therefore WE ∈ [0, 1] by construction.
    denom = float(np.log(B)) if B > 1 else 1.0
    H_vec = np.clip(H_vec_raw / denom, 0.0, 1.0)

    # WE (normalized) and occupancy entropy
    WE_raw = (P_occ.to_numpy() * H_vec_raw[None, :]).sum(axis=1)
    WE = (P_occ.to_numpy() * H_vec[None, :]).sum(axis=1)    # WE = sum_i p_i * H_i
    WE = np.clip(WE, 0.0, 1.0)
...
def main():
    ...
    np.savez_compressed(
        ...
        WE_raw=feat_df["WE_raw"].to_numpy(),
